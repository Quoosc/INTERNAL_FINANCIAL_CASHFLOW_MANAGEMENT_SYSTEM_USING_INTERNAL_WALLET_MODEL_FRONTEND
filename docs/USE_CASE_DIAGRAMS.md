# USE CASE DIAGRAMS — IFMS (Sequence & Activity)

## Tổng hợp danh sách Use Case

| STT | Mã UC     | Tên Use Case                              | Nhóm                        | Có SD + AD |
| --- | --------- | ----------------------------------------- | --------------------------- | ---------- |
| 1   | UC-CM-01  | Sign In                                   | Common                      | ✅         |
| 2   | UC-CM-02  | Manage Profile                            | Common                      | ✅         |
| 3   | UC-CM-03  | Manage Transaction PIN                    | Common                      | ✅         |
| 4   | UC-CM-04  | View Wallet & Transaction History         | Common                      | —          |
| 5   | UC-CM-05  | Deposit                                   | Common                      | ✅         |
| 6   | UC-CM-06  | Withdraw                                  | Common                      | ✅         |
| 7   | UC-CM-07  | View Payslip                              | Common                      | —          |
| 8   | UC-CM-08  | View Notifications                        | Common                      | —          |
| 9   | UC-EMP-01 | Manage Request (Flow 1 - Tạo)             | Employee                    | ✅         |
| 10  | UC-EMP-02 | View Active Projects                      | Employee                    | —          |
| 11  | UC-TL-01  | Approve/Reject Personal Expense (Flow 1)  | Team Leader                 | ✅         |
| 12  | UC-TL-02  | Request Project Top-up (Flow 2)           | Team Leader                 | —          |
| 13  | UC-TL-03  | Manage Project Phases & Budget            | Team Leader                 | —          |
| 14  | UC-TL-04  | Manage Project Members                    | Team Leader                 | —          |
| 15  | UC-MGR-01 | Manage Projects                           | Manager                     | —          |
| 16  | UC-MGR-02 | Approve/Reject Project Top-up (Flow 2)    | Manager                     | ✅         |
| 17  | UC-MGR-03 | Request Department Top-up (Flow 3)        | Manager                     | —          |
| 18  | UC-MGR-04 | View Department Dashboard                 | Manager                     | —          |
| 19  | UC-ACC-01 | Manage Disbursement (Flow 1)              | Accountant                  | ✅         |
| 20  | UC-ACC-02 | Manage Payroll                            | Accountant                  | ✅         |
| 21  | UC-ACC-03 | Manage System Fund                        | Accountant                  | —          |
| 22  | UC-ACC-04 | Approve Withdrawal                        | Accountant                  | —          |
| 23  | UC-ADM-01 | Manage Users                              | Admin                       | ✅         |
| 24  | UC-ADM-02 | Manage Departments                        | Admin                       | —          |
| 25  | UC-ADM-03 | Approve/Reject Department Top-up (Flow 3) | CFO (UI namespace `/admin`) | ✅         |
| 26  | UC-ADM-04 | Manage Roles & Permissions                | Admin                       | —          |
| 27  | UC-ADM-05 | Manage System Settings                    | Admin                       | —          |
| 28  | UC-ADM-06 | View Audit Logs                           | Admin                       | —          |
