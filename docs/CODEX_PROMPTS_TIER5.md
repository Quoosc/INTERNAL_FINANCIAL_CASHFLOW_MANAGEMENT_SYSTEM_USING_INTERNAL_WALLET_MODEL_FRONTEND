...
### ⚠️ Business rules
- Accountant giải ngân Flow 1 (PENDING_ACCOUNTANT → PAID), không duyệt YC
...
|--------|----------|--------|
| GET | `/api/v1/accountant/disbursements?limit=3&status=PENDING_ACCOUNTANT` | Sprint 6 |
...
const MOCK_PENDING_DISBURSEMENTS: DisbursementListViewItem[] = [
  {
    ...
    status: "PENDING_ACCOUNTANT", amount: 3_500_000, approvedAmount: 3_500_000,
  ...
  },
  {
    ...
    status: "PENDING_ACCOUNTANT", amount: 850_000, approvedAmount: 850_000,
  ...
  },
  {
    ...
    status: "PENDING_ACCOUNTANT", amount: 1_200_000, approvedAmount: 1_200_000,
...
### 🎯 Mục tiêu
Page `/accountant/disbursements`: danh sách requests đã được TL duyệt, đang chờ Accountant giải ngân (`PENDING_ACCOUNTANT`). Filter type + search. Click row → detail page (nhập PIN để giải ngân).
...
### ⚠️ Business rules
- Chỉ hiển thị `status = PENDING_ACCOUNTANT` (đã được TL approve, chờ Accountant)
...
const MOCK_DISBURSEMENTS: DisbursementListItem[] = [
  {
    id: 1, requestCode: "REQ-2026-0041", type: "ADVANCE",
    status: "PENDING_ACCOUNTANT", amount: 3_500_000, approvedAmount: 3_500_000,
    requester: { id: 11, fullName: "Đỗ Quốc Bảo", avatar: null, employeeCode: "EMP001", jobTitle: "Frontend Developer", email: "emp.it1@ifms.vn" },
    approver: { id: 4, fullName: "Hoàng Minh Tuấn", approvedAt: "2026-04-03T10:00:00" },
    projectCode: "PRJ-IT-001", projectName: "Hệ thống quản lý nội bộ",
    createdAt: "2026-04-03T09:15:00",
  },
  {
    id: 2, requestCode: "REQ-2026-0042", type: "EXPENSE",
    status: "PENDING_ACCOUNTANT", amount: 850_000, approvedAmount: 850_000,
    requester: { id: 12, fullName: "Vũ Thị Lan", avatar: null, employeeCode: "EMP002", jobTitle: "Backend Developer", email: "emp.it2@ifms.vn" },
    approver: { id: 4, fullName: "Hoàng Minh Tuấn", approvedAt: "2026-04-03T09:30:00" },
    projectCode: "PRJ-IT-001", projectName: "Hệ thống quản lý nội bộ",
    createdAt: "2026-04-02T14:30:00",
  },
  {
    id: 3, requestCode: "REQ-2026-0038", type: "REIMBURSE",
    status: "PENDING_ACCOUNTANT", amount: 1_200_000, approvedAmount: 1_200_000,
    requester: { id: 13, fullName: "Phạm Văn Đức", avatar: null, employeeCode: "EMP003", jobTitle: "QA Engineer", email: "emp.sales1@ifms.vn" },
    approver: { id: 4, fullName: "Hoàng Minh Tuấn", approvedAt: "2026-04-02T16:00:00" },
    projectCode: "PRJ-IT-002", projectName: "Nâng cấp hạ tầng mạng",
    createdAt: "2026-04-01T10:00:00",
  },
  {
    id: 4, requestCode: "REQ-2026-0035", type: "ADVANCE",
    status: "PENDING_ACCOUNTANT", amount: 5_000_000, approvedAmount: 4_500_000,
    requester: { id: 11, fullName: "Đỗ Quốc Bảo", avatar: null, employeeCode: "EMP001", jobTitle: "Frontend Developer", email: "emp.it1@ifms.vn" },
    approver: { id: 4, fullName: "Hoàng Minh Tuấn", approvedAt: "2026-04-01T14:00:00" },
    projectCode: "PRJ-IT-001", projectName: "Hệ thống quản lý nội bộ",
...
- Backend trả `DisburseResponse { transactionCode, amount, walletBalance }`
...
const MOCK_DETAIL: DisbursementDetailResponse = {
  ...
  requester: { id: 11, fullName: "Đỗ Quốc Bảo", avatar: null, employeeCode: "EMP001", jobTitle: "Frontend Developer", email: "emp.it1@ifms.vn", departmentName: "Phòng IT" },
  approver: { id: 4, fullName: "Hoàng Minh Tuấn", approvedAt: "2026-04-03T10:00:00", comment: "Đồng ý — đây là thiết bị cần thiết cho dự án" },
  projectCode: "PRJ-IT-001", projectName: "Hệ thống quản lý nội bộ",
  phaseCode: "PH-001", phaseName: "Phase 1 - Phân tích",
  ...
  timeline: [
    { id: 1, action: "APPROVE", statusAfterAction: "PENDING_APPROVAL", actorId: 11, actorName: "Đỗ Quốc Bảo", comment: null, createdAt: "2026-04-03T09:15:00" },
    { id: 2, action: "APPROVE", statusAfterAction: "PENDING_ACCOUNTANT", actorId: 4, actorName: "Hoàng Minh Tuấn", comment: "Đồng ý — đây là thiết bị cần thiết", createdAt: "2026-04-03T10:00:00" },
  ],
  createdAt: "2026-04-03T09:15:00",
};
```
...
2. **Approval chain** — timeline TL → Accountant:
   - Step 1: Employee tạo YC (createdAt)
   - Step 2: TL duyệt (approver.fullName + approvedAt + comment)
   - Step 3: Accountant (current — chờ xử lý, pulsing indicator)
3. Requester info: avatar placeholder + fullName + employeeCode + email + dept
4. Request details: mô tả + project/phase/category
...
3. **Nút "Từ chối"** (red outline, luôn hiển thị nếu status = PENDING_ACCOUNTANT)
...
- Run payroll: foreach payslip → SystemFund.debit → Wallet.credit → reduceDebt → Transaction(PAYSLIP_PAYMENT)
...
```typescript
import {
  TransactionResponse, TransactionFilterParams, TransactionType,
...
| Query params | `type`, `from`, `to`, `search`, `page`, `limit=20` | | ← `TransactionFilterParams` dùng `from`/`to` (YYYY-MM-DD), KHÔNG phải `startDate`/`endDate`

### 📊 Mock data
```typescript
// TODO: Replace when Sprint 7 is complete
// LedgerSummaryResponse: currentBalance, totalInflow, totalOutflow, transactionCount
// KHÔNG có: netFlow, periodLabel
...
// TransactionResponse: id, transactionCode, type: TransactionType, status: TransactionStatus,
// amount, balanceAfter, referenceType: ReferenceType|null, referenceId: number|null, description, createdAt
// KHÔNG có referenceCode — dùng local extension nếu cần
// TransactionStatus enum: SUCCESS | PENDING | FAILED (KHÔNG có COMPLETED)
// ReferenceType enum: REQUEST | PAYSLIP | PROJECT | DEPARTMENT | SYSTEM
type LedgerTransactionView = TransactionResponse & { referenceCode?: string | null };

const MOCK_TRANSACTIONS: LedgerTransactionView[] = [
  { id: 1, transactionCode: "TXN-2026-0001A", type: TransactionType.REQUEST_PAYMENT, amount: -3_500_000, status: TransactionStatus.SUCCESS, balanceAfter: 1_246_500_000, referenceId: 1, referenceType: ReferenceType.REQUEST, referenceCode: "REQ-2026-0041", description: "Giải ngân tạm ứng - Đỗ Quốc Bảo", createdAt: "2026-04-03T11:00:00" },
  { id: 2, transactionCode: "TXN-2026-0002B", type: TransactionType.PAYSLIP_PAYMENT, amount: -13_500_000, status: TransactionStatus.SUCCESS, balanceAfter: 1_233_000_000, referenceId: 101, referenceType: ReferenceType.PAYSLIP, referenceCode: "PS-2026-03-001", description: "Chi lương T03/2026 - Đỗ Quốc Bảo", createdAt: "2026-04-02T17:05:00" },
  { id: 3, transactionCode: "TXN-2026-0003C", type: TransactionType.DEPOSIT, amount: 500_000_000, status: TransactionStatus.SUCCESS, balanceAfter: 1_500_000_000, referenceId: null, referenceType: ReferenceType.SYSTEM, referenceCode: null, description: "Nạp tiền vào Quỹ hệ thống", createdAt: "2026-04-01T09:00:00" },
  { id: 4, transactionCode: "TXN-2026-0004D", type: TransactionType.DEPT_QUOTA_ALLOCATION, amount: -200_000_000, status: TransactionStatus.SUCCESS, balanceAfter: 1_300_000_000, referenceId: 20, referenceType: ReferenceType.DEPARTMENT, referenceCode: "DEPT-Q2-2026", description: "Cấp quota Phòng IT Q2/2026", createdAt: "2026-04-01T10:30:00" },
  { id: 5, transactionCode: "TXN-2026-0005E", type: TransactionType.PROJECT_QUOTA_ALLOCATION, amount: -50_000_000, status: TransactionStatus.SUCCESS, balanceAfter: 1_250_000_000, referenceId: 10, referenceType: ReferenceType.PROJECT, referenceCode: "REQ-2026-0050", description: "Cấp vốn DA PRJ-IT-001 Phase 2", createdAt: "2026-04-01T10:35:00" },
  { id: 6, transactionCode: "TXN-2026-0006F", type: TransactionType.WITHDRAW, amount: -2_000_000, status: TransactionStatus.SUCCESS, balanceAfter: 1_248_000_000, referenceId: null, referenceType: null, referenceCode: null, description: "Rút tiền - Vũ Thị Lan", createdAt: "2026-03-30T14:00:00" },
];
```
...
```typescript
import {
  TransactionDetailResponse, TransactionType, TransactionStatus,
...
```typescript
// TODO: Replace when Sprint 7 is complete
// TransactionDetailResponse extends TransactionResponse:
// + paymentRef, gatewayProvider, walletId, walletOwnerName, actorId, actorName, relatedTransactionId, updatedAt
// KHÔNG có: referenceCode, balanceBefore, ipAddress
// status = TransactionStatus.SUCCESS ("SUCCESS"), KHÔNG phải "COMPLETED"
// Dùng local extension để thêm referenceCode nếu cần
type LedgerTxnDetailView = TransactionDetailResponse & {
...
const MOCK_TXN: LedgerTxnDetailView = {
  ...
  paymentRef: null, gatewayProvider: PaymentProvider.INTERNAL_WALLET,
  walletId: 101, walletOwnerName: "Đỗ Quốc Bảo",
  actorId: 7, actorName: "Lê Văn Cường",
...
3. **Section 1 — Financials**:
   ...
   - Ví người nhận: walletOwnerName + walletId
   - Số dư sau GD: `balanceAfter` VND (TransactionDetailResponse không có balanceBefore)

4. **Section 2 — Source (Polymorphic)**:
   - Nếu `referenceType = REQUEST` → "Xem yêu cầu {referenceCode}" → link `/requests/{referenceId}` (referenceCode là UI-only, cần local extension type hoặc fetch từ API)
   - Nếu `referenceType = PAYSLIP` → "Xem phiếu lương {referenceCode}" → link `/payroll/{referenceId}`
   ...
   - Nếu `referenceType = null` → "Giao dịch trực tiếp"

5. **Section 3 — Audit & Traceability**:
   - Người thực hiện: actorName + actorId (có thể null)
   - Thời gian: createdAt (full timestamp)
   - GD đối ứng: `relatedTransactionId` — "Xem giao dịch đối ứng #{id}" (double-entry)
   - Gateway: `gatewayProvider` (nếu không null)
